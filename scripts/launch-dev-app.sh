...
bundle_dir="$HOME/Applications/Respect Island.app"
...
if security find-identity -p codesigning -v "$HOME/Library/Keychains/login.keychain-db" 2>/dev/null \
       | grep -q '"Respect Island Local"'; then
    sign_identity="Respect Island Local"
else
    echo
...
echo "Respect Island.app did not stay running after launch." >&2
exit 1
