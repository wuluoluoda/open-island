...
- **Claude Code 感知能力** — 支持 Claude Desktop Code tab 保活、本地 5 小时/7 天 usage windows、status-line 集成，以及 opt-in 的 token/cost 可见性。
...
- **Claude Code** — 基于 hook 的集成，通过 `~/.claude/settings.json` 配置。识别来自 `Claude.app` 的 Claude Desktop Code tab hooks，并在桌面端运行期间保持会话存活。从 `~/.claude/projects/` JSONL transcript 自动发现会话。跨应用启动持久化和恢复会话。受管 status line bridge，opt-in 安装。读取缓存的 5 小时和 7 天 usage windows。
