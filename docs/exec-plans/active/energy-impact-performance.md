...
`Respect Island` can show high long-running energy impact in Activity Monitor even when the island is mostly idle. The core island experience is display, notification, and jump-back; background work that does not directly support those paths should be quiet by default.
...
- Run `swift test`.
- Refresh `Respect Island.app` with `zsh scripts/launch-dev-app.sh`.
