...
How to cut a new GitHub release for Respect Island.
...
2. **Build & package**:
   ...
   ```
   This produces `output/package/Respect Island.dmg` and `output/package/Respect Island.zip`.
3. **Sign the update zip with EdDSA** (for Sparkle auto-update):
   ...
   .build/artifacts/sparkle/Sparkle/Sparkle.xcframework/macos-arm64_x86_64/Sparkle.framework/Versions/B/bin/sign_update \
     "output/package/Respect Island.zip"
   ```
...
```markdown
## Respect Island v<version> — <Title>
...
1. Download **Respect Island.dmg**, open it, and drag **Respect Island** to **Applications**.
   下载 **Respect Island.dmg**，打开后将 **Respect Island** 拖入 **Applications**。

2. Since this is an unsigned app, macOS will show **"Respect Island is damaged"** when you try to open it. Run this command in Terminal to fix it:
   由于应用未签名，macOS 会提示**「"Respect Island"已损坏」**。请在终端中执行以下命令：

   ```bash
   xattr -dr com.apple.quarantine "/Applications/Respect Island.app"
   ```
...
| File | Purpose |
|------|---------|
| `Respect Island.dmg` | Styled disk image with drag-to-Applications |
| `Respect Island.zip` | Plain zip for automation / CI downloads |
